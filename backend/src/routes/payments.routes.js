...
router.post('/topup', requireAuth, async (req, res) => {
    ...
    if (!Number.isFinite(numericAmount) || numericAmount <= 0) {
        return res.status(400).json({error: 'Неверное сумма пополнении.'});
    }
...
router.post('/:id/check-status', requireAuth, async (req, res) => {
    ...
    if (result.paymentStatus === undefined) {
        ...
        return res.json({
            status: 'PENDING',
            message:
                'Компания Click пока не предоставила никакой информации об этом платеже. Если вы только что совершили платеж, подождите несколько минут и попробуйте снова. Если проблема сохраняется, пожалуйста, свяжитесь с нами через раздел «Помощь».',
        });
    }
