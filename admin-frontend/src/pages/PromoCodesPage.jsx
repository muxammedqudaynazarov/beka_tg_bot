...
function CreateForm({ onCreated }) {
  ...
  async function submit() {
    ...
    try {
      ...
      setCode(''); setDiscountPercent(''); setTopupAmount('');
      setBonusPercent(''); setMaxRedemptions('');
      setReferralTelegramId(''); setReferralPercent('');
      onCreated();
    } catch (err) {
    ...
    <div className="space-y-2 rounded-lg border border-accent/30 bg-accent/5 p-3">
      ...
      {type === 'DISCOUNT' && (
        <div className="flex gap-2">
          <input type="number" min="1" max="100" value={discountPercent} onChange={(e) => setDiscountPercent(e.target.value)} placeholder="Скидка, %" className={inputCls} />
          <input type="number" min="1" value={discountUses} onChange={(e) => setDiscountUses(e.target.value)} placeholder="Использований" className={inputCls} />
        </div>
      )}
      {type === 'BALANCE_TOPUP' && (
        <input type="number" min="1" value={topupAmount} onChange={(e) => setTopupAmount(e.target.value)} placeholder="Сумма пополнения (сум)" className={inputCls} />
      )}
      {type === 'FIRST_DEPOSIT_BONUS' && (<>
        <input type="number" min="1" max="100" value={bonusPercent} onChange={(e) => setBonusPercent(e.target.value)} placeholder="Бонус пользователю, % от суммы первого пополнения" className={inputCls} />
        <div className="rounded-md border border-accent/20 bg-accent/5 p-2 space-y-1.5">
          ...
          <p className="text-[10px] text-muted">Укажите Telegram ID рекламщика (Айбека) и его долю. Когда кто-то активирует этот код и пополнит баланс — рекламщик автоматически получит свой процент.</p>
          <div className="flex gap-2">
            <input type="number" value={referralTelegramId} onChange={(e) => setReferralTelegramId(e.target.value)} placeholder="Telegram ID рекламщика" className={inputCls} />
            <input type="number" min="1" max="100" value={referralPercent} onChange={(e) => setReferralPercent(e.target.value)} placeholder="Его доля, %" className="w-32 shrink-0 rounded-md border border-border bg-surface px-2.5 py-1.5 text-xs text-ink placeholder:text-muted focus:border-accent focus:outline-none" />
          </div>
        </div>
      </>)}

      <input type="number" min="1" value={maxRedemptions} onChange={(e) => setMaxRedemptions(e.target.value)} placeholder="Лимит активаций (пусто = без лимита)" className={inputCls} />
        ...
        <div>
          ...
          {promo.referralTelegramId && (
            <p className="mt-0.5 text-[10px] text-warning">
              💰 Реферал: Telegram ID {String(promo.referralTelegramId)} · {Number(promo.referralPercent)}% от каждого пополнения
            </p>
          )}
        </div>
