...
function SectionTitle({children}) {
    return <h2
        className="mb-2 font-display text-xs font-bold uppercase tracking-wide text-ink-secondary">{children}</h2>;
}
...
export default function FilterPage() {
    ...
    return (
        <div className="min-h-screen pb-28">
            ...
            {loading ? (
                ...
                <main className="space-y-6 px-4 pt-5">
                    ...
                    {availableSubcategories.length > 0 && (
                        <section>
                            <SectionTitle>
                                {draft.categoryIds.length ? 'Категории' : 'Все скины'}
                            </SectionTitle>
                            <div className="flex flex-wrap gap-2">
                    ...
                    {/* Narx tartibi */}
                    <section>
                        <SectionTitle>Сортировать по цене</SectionTitle>
                        <div className="flex gap-2">
                            <button
                                onClick={() => setDraft((d) => ({
                                    ...d,
                                    sort: d.sort === 'price_asc' ? null : 'price_asc'
                                }))}
                            ...
                            <button
                                onClick={() => setDraft((d) => ({
                                    ...d,
                                    sort: d.sort === 'price_desc' ? null : 'price_desc'
                                }))}
                                className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-medium ${
            ...
            <div
                className="fixed inset-x-0 bottom-16 z-20 flex gap-3 border-t border-base-border bg-base-bg/95 px-4 py-3 backdrop-blur">
                <button onClick={clearAll}
                        className="flex-1 rounded-xl bg-base-surface py-2.5 font-display text-sm font-semibold text-ink-secondary">
                    Очистить
                </button>
                <button onClick={apply}
                        className="flex-1 rounded-xl bg-rarity-covert py-2.5 font-display text-sm font-semibold text-white">
                    Применить
                </button>
            </div>
        </div>
