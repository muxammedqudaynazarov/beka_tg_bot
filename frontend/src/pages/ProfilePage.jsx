...
function AwaitingPaymentRow({auction, onPaid}) {
    ...
    async function completePayment() {
        ...
        try {
            ...
            showAlert(err.response?.data?.error || 'Платеж не удалось завершить.');
        } finally {
            setPaying(false);
        }
    }
        ...
        return (
            <div className="rounded-xl bg-base-surface px-3.5 py-3">
                ...
                <p className="mt-1 flex items-center gap-1 text-[11px] text-signal-success">
                    <CheckCircle2 size={12}/> Оплачено — дождитесь отправки через Steam.
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="rounded-xl border border-signal-warning/40 bg-signal-warning/5 px-3.5 py-3">
            ...
            <p className="mt-1 flex items-center gap-1 font-mono text-[11px] text-signal-warning">
                <Clock size={11}/> Оплатить в течение <span key={countdown}
                                                            className="countdown-flash">{countdown}</span>
            </p>
            <button
...
export default function ProfilePage() {
    ...
    async function saveTradeUrl() {
        ...
        if (trimmed && !STEAM_TRADE_URL_RE.test(trimmed)) {
            showAlert(
                'Формат URL-адреса для обмена указан неверно. Скопируйте правильную ссылку из Steam > Инвентарь > Настройки предложений обмена.'
            );
            return;
        }
        ...
        try {
            ...
            showAlert(err.response?.data?.error || 'Произошла ошибка при сохранении.');
        } finally {
    ...
    return (
        <div className="min-h-screen px-4 pb-28 pt-6">
            <div className="mb-6 flex items-center gap-3">
                <div
                    className="flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-rarity-restricted to-rarity-covert font-display text-lg font-bold text-white">
                    {(user?.firstName || 'U')[0]}
                </div>
                <div>
                    <p className="font-display text-base font-bold text-ink-primary">{user?.firstName || 'Пользователь'}</p>
                    <p className="text-xs text-ink-secondary">@{user?.username || 'имя пользователя отсутствует'}</p>
                </div>
            </div>
            ...
            {Number(user?.discountPct) > 0 && (
                <div
                    className="mb-6 flex items-center gap-2 rounded-xl bg-signal-success/10 px-3.5 py-2.5 text-xs text-signal-success">
                    <ShieldCheck size={14}/>
                    Администратор предоставил вам скидку в размере <strong>{user.discountPct}%</strong>.
                </div>
            )}
            ...
            <div className="mb-6 space-y-2">
                <div
                    className="flex items-center gap-2 rounded-xl border border-base-border bg-base-surface px-3 py-2.5">
                ...
                >
                    {savingTradeUrl ? 'Сохраняется…' : 'Сохранять'}
                </button>
                <p className="text-[10px] text-ink-muted">
                    Выигранный вами скин будет отправлен в ваш инвентарь Steam по этой ссылке. Вы можете найти его в
                    настройках Steam &gt; Инвентарь &gt; Предложения по обмену.
                </p>
            </div>
            ...
            {purchases.length ? (
                <div className="mb-6 space-y-2">
                    {purchases.map((p) => (
                        <div key={p.id}
                             className="flex items-center justify-between rounded-xl bg-base-surface px-3.5 py-3">
                            <div>
                ...
                <p className="mb-6 text-xs text-ink-muted">Скины пока не куплены.</p>
            )}
